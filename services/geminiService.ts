...
const callGemini = async (system: string, user: string, jsonMode: boolean): Promise<string> => {
    ...
    // Fallback to fetch to avoid SDK build issues
    const model = currentConfig.geminiModel || 'gemini-1.5-flash';
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${currentConfig.geminiKey}`;
